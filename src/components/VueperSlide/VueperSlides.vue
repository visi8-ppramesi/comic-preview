<template>
  ...
  >
    ...
    >
      <vnodes
        v-if="currentSlide.contentSlot"
        :vnodes="currentSlide.contentSlot()"
      ></vnodes
      ><template v-else>
        <div
    ...
    <div class="vueperslides__inner">
      <div
        ...
        :style="`padding-bottom: ${conf.slideRatio * 100}%`"
        aria-live="polite"
      >
        <div
          ...
          ref="track"
          :class="{
            'vueperslides__track--dragging': touch.dragging,
            'vueperslides__track--mousedown': mouseDown,
          }"
          :style="trackStyles"
        >
          <div class="vueperslides__track-inner" :style="trackInnerStyles">
            ...
            <vueper-slide
              ...
              aria-hidden="true"
              ><template v-if="lastSlide.contentSlot">
                <vnodes :vnodes="lastSlide.contentSlot()"></vnodes> </template
              ><template
                v-if="conf.lazy && !lastSlide.loaded && lastSlide.loaderSlot"
              >
                <vnodes :vnodes="lastSlide.loaderSlot()"></vnodes> </template
            ></vueper-slide>
            <vueper-slide
              ...
              aria-hidden="true"
              ><template v-if="firstSlide.contentSlot">
                <vnodes :vnodes="firstSlide.contentSlot()"></vnodes> </template
              ><template
                v-if="conf.lazy && !firstSlide.loaded && firstSlide.loaderSlot"
              >
                <vnodes :vnodes="firstSlide.loaderSlot()"></vnodes> </template
            ></vueper-slide>
          </div>
        </div>
      </div>
      ...
      <div class="vueperslides__progress" v-if="conf.progress">
        ...
        >
          <div
            :style="`width: ${((slides.current + 1) * 100) / slidesCount}%`"
          ></div>
        </slot>
      </div>
          ...
          <slot :name="`arrow-${conf.rtl ? 'right' : 'left'}`">
            <svg viewBox="0 0 9 18">
              <path
                ...
                :d="conf.rtl ? 'm1 1 l7 8 -7 8' : 'm8 1 l-7 8 7 8'"
              ></path>
            </svg>
          </slot>
        </button>
        ...
        >
          <slot :name="`arrow-${conf.rtl ? 'left' : 'right'}`"
            ><svg viewBox="0 0 9 18">
              <path
                stroke-linecap="round"
                :d="conf.rtl ? 'm8 1 l-7 8 7 8' : 'm1 1 l7 8 -7 8'"
              ></path>
            </svg>
          </slot>
        </button>
      ...
      >
        <slot
          ...
          ><button
            ...
            :key="i"
            :class="{
              'vueperslides__bullet--active': slides.current === slideIndex,
            }"
            role="tab"
...
.vueperslides__bullets button::-moz-focus-inner, .vueperslides__bullet::-moz-focus-inner {
  border: 0;
}
